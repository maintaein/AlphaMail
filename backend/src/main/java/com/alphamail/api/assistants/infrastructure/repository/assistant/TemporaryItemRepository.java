...
public interface TemporaryItemRepository extends JpaRepository<UserEntity, Integer> {
	@Query(value =
		"SELECT id, 'PURCHASE_ORDER' as type, created_at as createdAt, title, user_id as userId " +
			"FROM temporary_purchase_order " +
			"WHERE user_id = :userId " +
			"UNION ALL " +
			"SELECT id, 'QUOTE' as type, created_at as createdAt, title, user_id as userId " +
			"FROM temporary_quote " +
			"WHERE user_id = :userId " +
			"UNION ALL " +
			"SELECT id, 'CLIENT' as type, created_at as createdAt, corp_name as title, user_id as userId " +
			"FROM temporary_client " +
			"WHERE user_id = :userId " +
			"UNION ALL " +
			"SELECT id, 'SCHEDULE' as type, created_at as createdAt, name as title, user_id as userId " +
